...
tags:
  ...
  - android
categories: Android
description: 本文章将介绍okhttp3的简单封装工具类
---
...
推荐使用**异步请求**，因为已经把网络请求自动放到子线程了，用起来稍简单。而同步请求还需要自己new Thread+handler来做，几乎和原始的网络请求没区别了。因此这里举例就只举异步请求了。
...
要注意的是获取数据后，**更新UI要在主线程中**。我这里使用的是`runOnUiThread`也可以用Handler。

用Handler后你会发现这一部分的代码很少了，因为只需要把获取的数据用Message发送到Handler处理就好了，这样就把代码分散开了

用`runOnUiThread`的好处是很直接，不过看起来代码都堆到一起，其实很方便的。
